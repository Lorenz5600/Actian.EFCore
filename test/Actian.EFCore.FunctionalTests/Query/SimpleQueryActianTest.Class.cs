...
{
    public partial class SimpleQueryActianTest : SimpleQueryRelationalTestBase
    {
        ...
        public virtual async Task Muliple_occurrences_of_FromSql_in_group_by_aggregate(bool async)
        {
            ...
            var query = context.DemoEntities
                .FromSqlRaw("SELECT * FROM DemoEntities WHERE Id = {0}", new IngresParameter { Value = 1 })
                .Select(e => e.Id);
...
""");
        ...
        protected class Context30478 : DbContext
        {
            ...
            public void Seed()
            {
                ...
                Database.ExecuteSqlRaw($"ALTER TABLE \"Entities\" SET (SYSTEM_VERSIONING = ON (HISTORY_TABLE = \"EntitiesHistory\"))");
            }
        }
...
""");
        ...
        public override async Task Null_check_removal_in_ternary_maintain_appropriate_cast(bool async)
        {
            ...
            AssertSql(
                """
SELECT CAST("f"."Taste" AS smallint) AS "Bar"
FROM "Food" AS "f"
""");
        }
...
SELECT "m"."Id", "m"."SomeDate"
FROM "MyEntities" AS "m"
WHERE "ModifyDate"("m"."SomeDate") = @__date_0
""");
        }
