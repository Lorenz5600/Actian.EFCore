...
{
    /// <summary>
    /// Detects the exceptions caused by Actian transient failures.
    /// </summary>
    public static class ActianTransientExceptionDetector
    {
        public static bool ShouldRetryOn([NotNull] Exception ex)
        {
            if (ex is IngresException sqlException)
            {
                foreach (IngresError err in sqlException.Errors)
                {
                    // TODO: Check Actian error codes
                    //switch (err.Number)
                    //{
